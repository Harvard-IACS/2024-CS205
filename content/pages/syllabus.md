...
Homework must be submitted on [Gradescope](https://canvas.harvard.edu/courses/128330/external_tools/101445?display=borderless) before the due date. The homework is released through the [CS205 class Git repository](https://code.harvard.edu/CS205/main).

All submissions must be uploaded in a **single** `pdf` file, completed with your solutions. The file must contain only solutions for the non-coding problems. The solution for the coding problems must be located on a private repository created under the [CS205 Organization](https://code.harvard.edu/CS205). You must be indicate at the end of the `pdf` file the link of the commit you want to submit. We will only grade that single commit.

All your results and visualizations based on computed data that the problem statement is asking for _must be communicated in the written report_ by means of tables, plots or other forms of display *(attached data files are not considered for grading)*. Object files and other binary data generated by compilers must be cleaned out before submission.  *Please do not rename any files in the handout.*


**Example:** The homework 1 handout can be found [here](https://code.harvard.edu/CS205/main/tree/master/homework/hw1). To upload your solution to [Gradescope](https://canvas.harvard.edu/courses/128330/external_tools/101445?display=borderless), submit the solution as a `pdf` file. The contents of the repository should be located on the repository as follows:
